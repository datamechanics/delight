...
/** A class responsible for sending messages to the Data Mechanics collector API
  *
  *   - Events are bufferized then send as gzip/base64 bulks as part of a Json
  *     Payload
...
class DelightStreamingConnector(sparkConf: SparkConf) extends Logging {
  ...
  /** 2 Http Clients are created to avoid threading conflicts
    *   - httpClient is used in the main thread by publishPayload() and
    *     sendAck()
  ...
  /** Send a "/heartbeat" request to Data Mechanics collector API ("the server")
    ...
    *   - Uses a dedicated httpClient to avoid collision with the one used by
    *     the main thread
  ...
  /** Add a Event to the event buffer
    *
    *   - Events are "flushed" when (1) there are more than `bufferMaxSize`
    *     messages in the buffer or (2) when `flush` is true
    *   - Flushed events are transferred to the pendingMessages queue, in order
    *     to be sent to the server
    *   - This method starts the polling thread (see method `start`) if it has
    *     not started yet
    *   - This method waits for all messages to be sent to the server if
  ...
  /** Remove all events from the event buffer and aggregate them into a payload
    * ready to be sent to the server
    *
    *   - This method waits for all events to be sent to the server if
    *     `blocking` set to true
    */
  ...
  /** Wait for all pending events to be sent to the server
    *
    *   - This method is called when an ApplicationEnd event is received. It
    *     forces Spark to wait for the last events to be sent to the server,
    *     before shutting down the context.
    *   - Check every `pollingInterval` seconds that the queue of pending events
    *     is empty
  ...
  /** Serialize a SparkListenerEvent and drop it if it can't be serialized
    ...
    *
    * com.fasterxml.jackson.databind.JsonMappingException: Exceeded 2097152
    * bytes (current = 2100278) (through reference chain:
    * org.apache.spark.sql.catalyst.expressions.AttributeReference["canonicalized"]->org.apache.spark.s...
    * ... Caused by:
    * com.databricks.spark.util.LimitedOutputStream$LimitExceededException:
    * Exceeded 2097152 bytes (current = 2100278)
    */
  ...
  /** Send all pending events to the server.
    *
    *   - Stop sending pending events when a call has failed. In this case, time
    *     before next call to `publishPendingEvents` is doubled (exponential
    *     retry)
    *   - Time before next call to `publishPendingEvents` is reset to its
    *     initial value if a call has worked
    */
  ...
  /** Start the polling thread that sends all pending payloads to the server
    * every `currentPollingInterval` seconds. Start the heartbeat thread that
    * `sendHeartbeat()` every `heartbeatInterval` seconds.
    *
...
object DelightStreamingConnector {
  ...
  /** A connector common to the whole Scala application.
    *
    *   - Will become useful if we have more than a SparkListener sending
    *     messages!
    */
