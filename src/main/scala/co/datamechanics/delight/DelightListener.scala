...
class DelightListener(sparkConf: SparkConf) extends SparkListener with Logging {
  ...
  /** Activates memory metrics collection for Spark 3.0.0 and above. For Spark
    * versions below 3.0.0, these configs have no effect.
    */
  ...
  /** The listener creates the logStart event itself because Spark does not give
    * it to the listener. This a Spark quirk!
    */
