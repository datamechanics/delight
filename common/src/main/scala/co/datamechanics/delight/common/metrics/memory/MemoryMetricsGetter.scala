...
// This is a port of MemoryMetricsGetter from org.apache.spark.executor
// This will allow us to monitor memory on the driver
class MemoryMetricsGetter(procfsDir: String = "/proc/") extends Logging {
  ...
  private def addMemoryMetricsFromOneProcess(
  ...
  def computeAllMetrics(): MemoryMetrics = {
    if (!isAvailable) {
      return MemoryMetrics(0, 0, 0, 0, 0, 0)
    }
    val pids = computeProcessTree
    var allMetrics = MemoryMetrics(0, 0, 0, 0, 0, 0)
    for (p <- pids) {
      try {
        allMetrics = addMemoryMetricsFromOneProcess(allMetrics, p)
...
object MemoryMetricsGetter {
  final val MemoryMetricsGetter = new MemoryMetricsGetter

  def get(): MemoryMetricsGetter = {
    MemoryMetricsGetter
  }
}
